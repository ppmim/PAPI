...
! Version for PC and similar platforms (byte-swapping)
! 
! The source file (crt_nics5.f) is compiled by
!
!   f77 -o crt_nics crt_nics5.f 
! or
!   g77 -o crt_nics crt_nics5.f 
!
! To run it use 
!
...
!
      do j=1,512
	do i=1,512
	  araw(i,j)=real_from_i2swap(ima2(i,j))
	  braw(i,j)=real_from_i2swap(ima2(i+512,j))
	  craw(i,j)=real_from_i2swap(ima2(i,j+512))
	  draw(i,j)=real_from_i2swap(ima2(i+512,j+512))
	  acor(i,j)=araw(i,j)
	  bcor(i,j)=braw(i,j)
	  ccor(i,j)=craw(i,j)
	  dcor(i,j)=draw(i,j)
...
!
      do j=1,512
        do i=1,512
	  ima2(i,j)=i2swap_from_real(acor(i,j))
	  ima2(i+512,j)=i2swap_from_real(bcor(i,j))
	  ima2(i,j+512)=i2swap_from_real(ccor(i,j))
	  ima2(i+512,j+512)=i2swap_from_real(dcor(i,j))
        enddo
      enddo
...
!
	...
	enddo
      ...
      real function real_from_i2swap(i2)
!
! Do byte swapping of input I*2 variable
!
      integer*2 i2,j2
      character*1 b1(2),bb
      equivalence(b1(1) , j2)
      ...
      b1(1)=b1(2)
      b1(2)=bb
      real_from_i2swap=float(j2)
      return
      end
      
      integer*2 function i2swap_from_real(x)
!
! Do byte swapping of input I*2 variable
!
      integer*2 j2
