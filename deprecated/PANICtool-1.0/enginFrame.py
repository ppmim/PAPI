...
tasknames = ['subtractSkyN', "sumFrames", "subtractFrames", "test","test2" ] 
...
class window(Frame):
  ...
  def makeFrame(self):
    ...
    # of input options.
    optionlist = ('inpath', 'filename_filter')
    ...
    label = Label(self, text='Engineering tasks :')
    ...
    # At the bottom, display an 'Button1' button that will run the task2
    self.button1 = Button(self.root, text='testThread', command=testThread, bg='grey')
    self.button1.grid(row=i, rowspan=1, columnspan=3, sticky=W)
    ...
    self.okbutton = Button(self.root, text='OK', command=self.hide, bg='green')
  ...
  def _doTask(self, task, GUIupdater, mode):
    ...
    ##########################################################
    errorstring = taskManager.runTask(task, self.root)
    ##########################################################
...
def testThread():
    ...
    #tasks.createMasterDomeFlat().run()

    #tasks.createMasterDark().run()
    

    frame1="/disk-a/caha/panic/DATA/data_mat/QL1/orion0021.fits"
    t1=tasks.myThread1(frame1)
    t1.start()

    #sleep(10)
    
    #frame2="/disk-a/caha/panic/DATA/data_mat/QL1/orion0022.fits"
    #t2=tasks.myThread2(frame2)
    #t2.start()
    ...
    messageLog.put("Test threading finished..%s" % c.tac())
