...
""" Scatter-plot the data points (x, y, FWHM) of the four detectors and fit a
regression plane for each one of them, and an additional plane that fits all
the points, regardless of their plate """
...
if __name__ == "__main__":
    ...
    for nplate in range(1, 5):
        ...
        # Ignore plate 1 if comparison plane if this is
        # 3-detector-focus-cycle-data/ (where we have created this file)
        if nplate == 1 and os.path.exists("./.is-3-detecttor"):
            print()
            print("Plate", nplate, "not included in comparison plane")
        else:
            all_points += points
...
#        ax.plot_surface(xx, yy, z, alpha=0.2, color=[0,1,0])
    ...
    # Now fit plane to all the points
    ...
    print()
    print("All plates -- Equation:".format(nplate))
    print("A = {0}".format(a))
    print("B = {0}".format(b))
    print("C = {0}".format(-1))
    print("D = {0}".format(-d))
    ...
    xx, yy = np.meshgrid([min(x), max(x)], [min(y), max(y)])
    z = (-normal[0] * xx - normal[1] * yy - d) * 1. /normal[2]
    ax.plot_surface(xx, yy, z, alpha=0.15, color='red')
