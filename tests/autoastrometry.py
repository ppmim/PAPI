...
#    last significant modifications 2011-04-14
#  
#  Installation:
...
def sextract(sexfilename, nxpix, nypix, border=3, corner=12, minfwhm=1.5, maxfwhm=25, maxellip=0.5, saturation=-1):
    ...
    if len(fwhmlist) > 5:
       fwhmlist.sort()
       fwhm20 = fwhmlist[len(fwhmlist)/5]
       fwhm25 = fwhmlist[len(fwhmlist)/4]
...
overwrite=False, outfile='', saturation=-1, quiet=False):
    ...
    rot = offpa * math.pi/180
      #...the image itself
    h.update("CD1_1", cos(rot)*cd11 - sin(rot)*cd21 )
    h.update("CD1_2", cos(rot)*cd12 - sin(rot)*cd22 )  # a parity issue may be involved here?
    h.update("CD2_1", sin(rot)*cd11 + cos(rot)*cd21 )
    h.update("CD2_2", sin(rot)*cd12 + cos(rot)*cd22 )
    ...
    try:
       ...
       pass
    h.update("ASTR_CAT", catalog, "Reference catalog for autoastrometry")
