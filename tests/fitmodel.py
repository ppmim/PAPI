...
__cvs_id__ = "$Id: fitmodel.py,v 1.1 2010-04-28 01:00:50 dapple Exp $"
...
def f_test_exceeds_prob(chisq_old, dof_old, chisq_new, dof_new):
    ...
    F = (chisq_old - chisq_new)/((dof_new - dof_old)*chisq_new/dof_new)

    return stats.f.sf(F, dof_old, dof_new)
...
class FitModel:
    ...
    """
    def __init__(self, xdata, ydata, yerr, model, statfunc, guess = []):
...
class fake_code:
    """
    Create a fake func_code given a tuple of parameter names.
    """
    def __init__(self, *params):
        self.co_argcount = len(params)
        self.co_varnames = tuple(params)
...
class FCN:
    ...
    """
    def __init__(self, xdata, ydata, yerr, model, statfunc):
        self.xdata = xdata
        self.ydata = ydata
        self.yerr = yerr
        self.model = model
        ...
        if inspect.isfunction(self.model):
            self.params = inspect.getargspec(self.model)[0][1:]
        elif hasattr(self.model, '__call__'):
            args = inspect.getargspec(self.model.__call__)[0]
            if len(args) < 3:
                paramAttr = inspect.getargspec(self.model.__call__)[1]
                self.params = getattr(self.model, paramAttr)
            else:
                self.params = args[3:]

        # set the func_code to fool the Minuit constructor
        self.func_code = fake_code(*self.params)

    def __call__(self, *params):
        return self.statfunc(self.ydata, self.yerr, self.model(self.xdata, *params))
