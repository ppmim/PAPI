...
int Minimum(int n_smallest)
{
  ...
  if(n_smallest == 1)
      ...
      for(loop_y=0 ; loop_y<PIX_AXIS ; loop_y++)
	{
      ...
      printf("\nThe CPU-time for the sky determination (in minimum mode) was: %6.2f seconds", \
	     ((float)time_2-(float)time_1)/1000000);


      printf("\nThe minimum was %d times determined, this is a percentage of: %d\n", test, test*100/TOT_PIX);
  ...
  for(loop_y=0 ; loop_y<PIX_AXIS ; loop_y++)
      ...
      for(loop_x=0 ; loop_x<PIX_AXIS ; loop_x++)
	{
	  ...
	  qsort(column , TOT_IMAGES, sizeof(column[0]), compar);
  ...
  printf("\nThe CPU-time for the sky determination (in minimum average mode) was: %6.2f seconds", \
	 ((float)time_2-(float)time_1)/1000000);
