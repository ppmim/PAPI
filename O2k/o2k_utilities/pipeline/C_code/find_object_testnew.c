...
.VERSION		1.00   		22.12.02
			...
			4.1			27.03.03	add cosmics cut for threshold adjust-
								ment, add if backcut too high
-----------------------------------------------------------------------------------------*/
...
/* prototypes */
int Analyze(int x_pix, int y_pix, int stack_pos, float back_cut);
...
int Object(int n_average)
{
  ...
  int pix_count = 0;					/* count pixels above threshold */
  ...
  if(sum.total_opened <= 1)	
      ...
      do		/* until appropriate number of objects is found */
	{
		  ...
		  if(cosmics_stack[0][loop_y][loop_x] > threshold)
		    {
		      ...
		      /* check 5x5 neighborhood for more pixels above threshold */
		      for(sub_y = loop_y-2 ; sub_y <= loop_y+2 ; sub_y++)
			{
			  for(sub_x = loop_x-2 ; sub_x <= loop_x+2 ; sub_x++)
			    {
			      if(cosmics_stack[0][sub_y][sub_x] > threshold)	
				pix_count++;
			    }
			}
		      

		      /* count as candidate of several object pixels were found */
		      if(pix_count >= 2)
			n_candidates++;
	  ...
	  sprintf(aux_string,"Number of candidates: %d at threshold %f", n_candidates, threshold);
	  fstat = SCTPUT(aux_string);
      ...
      for(loop_y = y_start ; loop_y <= y_end ; loop_y++)
	{
	  for(loop_x = x_start ; loop_x <= x_end ; loop_x++)
	      ...
	      if(cosmics_stack[0][loop_y][loop_x] > object_threshold)
		{
		  ...
		  /* set background cut to threshold if it is too high */
		  //if(background_cut > object_threshold)
		  //  background_cut = object_threshold;
		  ...
		  fstat = Analyze(loop_x , loop_y, 0, background_cut);
		  ...
		  if(cosmics_stack[stack_pos][loop_y][loop_x] > object_threshold)
		    {
		      ...
		      fstat = Analyze(loop_x , loop_y, stack_pos, background_cut);
...
int Analyze(int x_pix, int y_pix, int stack_pos, float back_cut)
{
  ...
  /* FWHM and total intensity are stored in current position of structure candidates */
  ...
  for(func_y = (y_high-10) ; func_y <= (y_high+10) ; func_y++)
    {
      for(func_x = (x_high-10) ; func_x <= (x_high+10) ; func_x++)
	{
	  ...
	  if(intensity > 0)
	    {
	      /* total intensity */	 
	      tot_intensity += cosmics_stack[stack_pos][func_y][func_x];
  ...
  candidates[top_cand].x_pos = cm_x + x_high;		/* absolute position in x */
  candidates[top_cand].y_pos = cm_y + y_high;		/* absolute position in y */
  ...
  candidates[top_cand].intensity = tot_intensity;
