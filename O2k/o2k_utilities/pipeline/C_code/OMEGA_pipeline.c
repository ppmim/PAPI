...
.VERSION		1.00		26.08.02
			...
			4.4		17.04.03	corrected bad pixel routine
			4.5		03.06.03	final corrections: use non-busy delay, 
							corrected output of last sum_image,
							prevent summation if clean stack not full
			4.6		02.06.04	comment out error message SCESIG(),
							not known to MPE compiler
-----------------------------------------------------------------------------------------*/
...
float bpm_aux[PIX_AXIS][PIX_AXIS];			/* 2D auxiliary array for bad pixel corr */
...
SUM_PAGE rejection[500];				/* declare 'rejection' as a structure for */
...

{
  ...
  for( ; ; )								/* endless loop */
      ...
      if(loose_end != 2)						/* true if new image availabe */
	{
	  ...
	  /* the end-of-catalog-handling is done in 3.10.16 */
		...
		{	
		  ...
		  stat = Tosum(SUM_SAVE_FLAG, N_AVERAGE, N_AVERAGE);
      ...
      if(ACTION_FLAG == 1)
	{
	  ...
	  /* of unused images on cosmics_stack --> if fully loaded */
	  if(clean.top_stack > 0 && sum.total_opened >= N_AVERAGE)   	    
	    {
	      stat = Cleansum(N_AVERAGE);
	      ...
	      stat = Tosum(0, clean.top_stack, N_AVERAGE);	
	  ...
	  /*(sum_save_flag = 1 or 2) --> correction: also for sum_save_flag = 0 */
	  else if(clean.top_stack == 0)
	    {
	      /* arg1 = 0 --> frame is written to output */
	      /* new frames in position: 0,1,...,top_stack-1 */
	      stat = Tosum(0, 0, N_AVERAGE);	
