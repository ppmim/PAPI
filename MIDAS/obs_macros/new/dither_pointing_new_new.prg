...
define/local expstarx/r/1/20 0 all 
define/local expstary/r/1/20 0 all
expstarx(1) = refstarx
expstary(1) = refstary

DO i = 2 20 
    l = i - 1
    expstarx({i}) = expstarx({l}) + x_offset({i})/0.44942 /10  
    expstary({i}) = expstary({l}) - y_offset({i})/0.44942 /10
ENDDO
...
if tel_flag(1:2) .eq. "AQ"  then
   ...
   if xy_test .ne. 0 then
    ...
    expstary(1) = refstary
        ...
        DO i = 2 20 
        expstarx({i}) = expstarx({i}) + x_repetition({ic})/0.44942 /10  
        expstary({i}) = expstary({i}) - y_repetition({ic})/0.44942 /10
        ENDDO
...
if P7(1:1) .EQ. "Y" then
    if refstarx .GE. 10 .AND. refstarx .LE. 2038 then
       if refstary .GE. 10 .AND. refstary .LE. 2038 then
           ...
           @@ O2K_UTIL:/obs_macros/new/autoguide_new {frame} {refstarx} {refstary} {expstarx({counter})} {expstary({counter})}
           x_autooff = {Q1}
           y_autooff = {Q2}
...
!
!
refstarx = {expstarx({counter})}
refstary = {expstary({counter})}
!
!
