...
if P1(1:4) .eq. "help"  then
   ...
   write/out "P8 Saturation level in counts [20000]"
   write/out
   goto exit
endif
...
define/par P7 s C/A "Save option [single/integrated] : "
define/par P8 20000 N/A "Saturation level : "
...
$ /mpiwork/caha/bin/ffltest
$ echo $status ! | write/key i
...
do i = 1 {n_ima}
   ...
   if i .eq. 1  then
      $cmd_panic_new median -stdout -raw | write/key curr_level/r/1/2
      act_level = curr_level(2)-curr_level(1)
      diff_level = act_level
      if curr_level(2) .gt. saturation  then
         write/out
