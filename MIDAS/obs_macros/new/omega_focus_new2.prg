...
define/parameter P1 focus ?         ! root
...
DEFINE/LOCAL nameexist/i/1/1 0
 

   
if {P6(1:1)} .eq. 0 then 

nameexist = m$exist("{P1}{start_index}.fits") 
    if nameexist .eq. 1 then
        DEFINE/LOCAL rootname/c/1/80
        Loop:
        INQUIRE/KEY rootname " <{P1}{start_index}.fits> already exists! Please choose another root name(e.g. focus17):" 
        WRITE/KEY P1 {rootname}
         nameexist = m$exist("{P1}{start_index}.fits")
         if nameexist .eq. 1 then 
         GOTO Loop 
         endif
 ...
 write/out 	{imagename1}
 write/out 	{masterframe}
...
SET/FORMAT I4
! set master back to identify galaxies
master_index = m$nint(makeodd)+{start_index}
write/keyword masterframe {root_save}{master_index}.fits
