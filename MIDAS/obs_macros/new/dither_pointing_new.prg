...
DEFINE/LOCAL refstarx/i/1/1 0
DEFINE/LOCAL refstary/i/1/1 0
...
! user input from file (extension ".dat")
   ...
   enddo
   close/file {fctrl(1)}
   write/out "Offsets from file {P6}.dat were defined ..."
...
if tel_return .ne. 0 then
 ...
 !AK
  ...
  if P7(1:1) .EQ. "Y" then
	...
	write/file {fctrl(1)} {isodate} "includes autoguide-correction" {x_autooff} {y_autooff} {loop} obj
     else 
	write/file {fctrl(1)} {isodate} "autoguide was skipped: no correction"
     endif
  endif			
endif	
...
if P7(1:1) .EQ. "Y" then
	if refstarx .GE. 10 .AND. refstarx .LE. 2038 then
	   if refstary .GE. 10 .AND. refstary .LE. 2038 then
		   ...
		   @@ O2K_UTIL:/obs_macros/new/autoguide {frame} {refstarx} {refstary}
		   x_autooff = {Q1}
		   y_autooff = {Q2}
