...
if {inputi(1)} .eq. 0 then 
   ...
   ! Apply undistortion to xi and eta
   ! convert radius -> mm 
   ! note dist is in %

   comp/tab {p1} :radius = 125/9.1e-3*sqrt(:xi**2+:eta**2)
   comp/tab {p1} :cb = abs(:xi/(sqrt(:xi**2+:eta**2)))
   comp/tab {p1} :sb = abs(:eta/(sqrt(:xi**2+:eta**2)))
   !comp/tab {p1} :dist = 9.7947e-3-8.2364e-4*:radius+1.0516e-4*:radius**2
   comp/tab {p1} :dist = 0.979909E-04*:radius**2
   comp/tab {p1} :xidist = :dist*:cb/100.
   comp/tab {p1} :etadist = :dist*:sb/100.
   comp/tab {p1} :Xidis = :xi*(1.-:xidist)  
   comp/tab {p1} :Etadis = :eta*(1.-:etadist)  
   loa/tab {p1} :Xidis :Etadis ? 1 4 6
   write/out undistorted positions ...  yellow
endif
...
write/out tables joined: {p1}temp + {p1} -> {p1}cat
