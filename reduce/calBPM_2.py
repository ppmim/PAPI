...
class BadPixelMask(object):
    """
    Builds a bad pixel mask (of dead pixels) from a set of images 
    ( lamp_on and lamp_off frames and darks) using iraf.ccdmask task.
    In addition to the flats, the proper dark is needed.
    ...
    def create_IRAF(self):
        ...
        """
        Generates a Bad Pixel Mask (.pl IRAF format - pixel list)
        ...
        for file in flats_off_frames:
            ...
            iraf.mscred.mscarith(operand1=file.replace("//", "/"),
                        ...
                        op='-',
                        result=file.replace("//","/").replace(".fits","_D.fits"),
                        )
        ...
        iraf.mscred.mscarith(operand1=flat_off_comb.replace('//','/'),
                 ...
                 op='/',
                 result=flat_ratio.replace('//','/'),
                 )
        log.debug("Flat Ratio created: %s "%flat_ratio)
    ...
    def create_simple(self):
        ...
        - Image file (FITS) with 0 for good pixels, non-zero for bad pixels (masked).
        pixels.
        ...
        for file in flats_off_frames:
            ...
            iraf.mscred.mscarith(operand1=file.replace('//','/'),
                        operand2=self.master_dark.replace('//','/'),
                        op='-',
                        result=file.replace(".fits","_D.fits").replace('//','/'),
                        )
            flats_off_frames[flats_off_frames.index(file)] = file.replace(".fits","_D.fits")
        ...
        nx1 = fr[int(nExt > 1)].header['NAXIS1']
        nx2 = fr[int(nExt > 1)].header['NAXIS2']
        ...
        for i in range(0, nExt):
            ...
            log.info("Fraction Bad pixel (extesion %d): %f"%(i + 1, float((bpm[i] == 1).sum()) / float(nx1 * nx2)))
        fr.close()
        ...
        prihdu.header.add_history('BPM created from %s' % (flats_on_frames + flats_off_frames))

        if nExt > 1:
            prihdu.header.set('EXTEND', True, after = 'NAXIS')
            prihdu.header.set('NEXTEND', nExt)
            prihdu.header.set('FILENAME', self.output_file)
